...
public class TextArea extends Widget {
	...
	int maxLength = 0;
	private float displayTextY;

	public TextArea (String text, Skin skin) {
		this(text, skin.get(TextAreaStyle.class));
	}

	public TextArea (String text, Skin skin, String styleName) {
		this(text, skin.get(styleName, TextAreaStyle.class));
	}

	public TextArea (String text, TextAreaStyle style) {
		setStyle(style);
	...
	private void initialize () {
		addListener(inputListener = new ClickListener() {
			...
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				...
				if (stage != null) stage.setKeyboardFocus(TextArea.this);
				keyboard.show(true);
				return true;
			}
	...
	/** Returns the text field's style. Modifying the returned style may not have an effect until {@link #setStyle(TextAreaStyle)}
	 * is called. */
	...
	*/
	...
	public void draw (SpriteBatch batch, float parentAlpha) {
		...
		if (background != null) {
			...
			textY = (int)(textY + (height - background.getTopHeight() - bottom ) );
		} else
	...
	/** Copies the contents of this TextArea to the {@link Clipboard} implementation set on this TextArea. */
	public void copy () {
		if (hasSelection) {
			int minIndex = Math.min(cursor, selectionStart);
			int maxIndex = Math.max(cursor, selectionStart);
	...
	/** Copies the selected contents of this TextArea to the {@link Clipboard} implementation set on this TextArea, then removes
	 * it. */
	public void cut () {
		if (hasSelection) {
			copy();
	...
	/** Pastes the content of the {@link Clipboard} implementation set on this TextArea to this TextArea. */
	void paste () {
	...
	/** Focuses the next TextArea. If none is found, the keyboard is hidden. Does nothing if the text field is not in a stage.
	 * @param up If true, the TextArea with the same or next smallest y coordinate is found, else the next highest. */
	public void next (boolean up) {
		Stage stage = getStage();
		if (stage == null) return;
		getParent().localToStageCoordinates(tmp1.set(getX(), getY()));
		TextArea textField = findNextTextArea(stage.getActors(), null, tmp2, tmp1, up);
		if (textField == null) { // Try to wrap around.
			if (up)
				tmp1.set(Float.MIN_VALUE, Float.MIN_VALUE);
			else
				tmp1.set(Float.MAX_VALUE, Float.MAX_VALUE);
			textField = findNextTextArea(getStage().getActors(), null, tmp2, tmp1, up);
		}
		if (textField != null)
	...
	private TextArea findNextTextArea (Array<Actor> actors, TextArea best, Vector2 bestCoords, Vector2 currentCoords, boolean up) {
		for (int i = 0, n = actors.size; i < n; i++) {
			Actor actor = actors.get(i);
			if (actor == this) continue;
			if (actor instanceof TextArea) {
				TextArea textField = (TextArea)actor;
				...
				best = findNextTextArea(((Group)actor).getChildren(), best, bestCoords, currentCoords, up);
		}
		return best;
	}

	/** @param listener May be null. */
	public void setTextAreaListener (TextAreaListener listener) {
		this.listener = listener;
	}

	/** @param filter May be null. */
	public void setTextAreaFilter (TextAreaFilter filter) {
		this.filter = filter;
	}
	...
	public void setText (String text) {
		...
		TextBounds bounds = font.getWrappedBounds(displayText, getWidth() - textOffset);
		if(bounds.height > getHeight()) {
			float visibleHeight = getHeight();
			float excessHeight = bounds.height - visibleHeight;
			float lineHeight = font.getLineHeight();
			int ceil = (int)(Math.ceil(excessHeight/lineHeight));
			//displayTextY = (ceil * lineHeight);
			
			
			int availableLines = (int)Math.ceil(visibleHeight / lineHeight);
			//get only the new append texts.
	...
	public TextAreaFilter getTextAreaFilter () {
		return filter;
	}
	 ...
	 * @author mzechner */
	static public interface TextAreaListener {
		public void keyTyped (TextArea textField, char key);
	}

	/** Interface for filtering characters entered into the text field.
	 * @author mzechner */
	static public interface TextAreaFilter {
		/** @param textField
		 * @param key
		 * @return whether to accept the character */
		public boolean acceptChar (TextArea textField, char key);

		static public class DigitsOnlyFilter implements TextAreaFilter {
			@Override
			public boolean acceptChar (TextArea textField, char key) {
	...
	/** The default {@link OnscreenKeyboard} used by all {@link TextArea} instances. Just uses
	...
	/** The style for a text field, see {@link TextArea}.
	 * @author mzechner
	 * @author Nathan Sweet */
	static public class TextAreaStyle {
		...
		public TextAreaStyle (BitmapFont font, Color fontColor, Drawable cursor, Drawable selection, Drawable background) {
